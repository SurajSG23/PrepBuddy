  ...
  return (
    <div className={`min-h-screen p-6 transition-colors duration-300 ${darkMode ? "bg-gradient-to-b from-gray-900 to-black text-white" : "bg-gradient-to-b from-indigo-50 to-white text-gray-900"}`}>
      <div className="max-w-5xl mx-auto">
        <h1 className={`text-4xl font-extrabold mb-6 text-center ${darkMode ? "text-yellow-300" : "text-yellow-700"}`}>JavaScript Practice Sheet</h1>
          ...
          <div className="flex gap-2">
            {levels.map((lv) => (
              <button
                ...
                className={`px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200 ${filter === lv ? (darkMode ? "bg-yellow-500 text-black shadow-md" : "bg-yellow-400 text-black shadow-md") : (darkMode ? "bg-gray-800 text-gray-200" : "bg-indigo-100 text-gray-900")}`}
              >
                {lv}
              </button>
            ))}
        ...
        <div className="space-y-4">
          ...
          {filtered.map((q) => (
            <div
              key={q.id}
              className={`p-4 rounded-2xl border shadow-sm transition-all ${darkMode ? "bg-gradient-to-br from-gray-800/80 to-gray-700/80 border-gray-600" : "bg-gradient-to-br from-yellow-50/80 to-white/80 border-yellow-200"}`}
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className={`text-lg font-semibold ${darkMode ? "text-yellow-200" : "text-yellow-700"}`}>{q.title}</span>
                    <span
                      className={`text-xs font-semibold px-2 py-1 rounded-full ${q.level === "Beginner" ? (darkMode ? "bg-green-600 text-white" : "bg-green-200 text-green-900") : q.level === "Intermediate" ? (darkMode ? "bg-blue-600 text-white" : "bg-blue-200 text-blue-900") : (darkMode ? "bg-red-600 text-white" : "bg-red-200 text-red-900")}`}
                    >
                      {q.level}
                    </span>
                    {q.tags && q.tags.length > 0 && (
                      <div className="ml-2 flex flex-wrap gap-2">
                        {q.tags.slice(0, 3).map((t) => (
                          <span key={t} className={`text-xs px-2 py-0.5 rounded ${darkMode ? "text-gray-300 bg-gray-800/50" : "text-gray-700 bg-yellow-100/50"}`}>{t}</span>
                        ))}
                      </div>
                    )}
                  </div>
                ...
                <div className="flex flex-col items-end gap-2">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className={`p-2 rounded-md transition-colors duration-200 ${darkMode ? "bg-gray-800 hover:bg-gray-700" : "bg-yellow-100 hover:bg-yellow-200"}`}
                    >
              ...
              {expandedId === q.id && (
                <div className="mt-4">
                  <div className={`border rounded-lg overflow-hidden ${darkMode ? "border-gray-700" : "border-yellow-200"}`}>
                    <CodeMirror
                      value={codeMap[q.id]}
                      height="200px"
                  ...
                  <div className="flex flex-wrap gap-3 items-center mt-3">
                    <button
                      ...
                      className={`inline-flex items-center gap-2 px-3 py-2 rounded-md font-semibold transition-colors duration-200 ${darkMode ? "bg-gradient-to-r from-yellow-500 to-yellow-400 text-black" : "bg-gradient-to-r from-yellow-400 to-yellow-300 text-black"}`}
                    >
                      <FiPlay /> Run Code
                    </button>
                    ...
                    <button
                      ...
                      className={`inline-flex items-center gap-2 px-3 py-2 rounded-md transition-colors duration-200 ${darkMode ? "bg-gray-800 border border-gray-600 text-gray-200" : "bg-yellow-100 border border-yellow-200 text-gray-900"}`}
                    >
                      <FiRotateCw /> Reset
                    </button>

                    <div className={`ml-auto text-sm ${darkMode ? "text-gray-400" : "text-gray-600"}`}>Example ID: {q.id}</div>
                  </div>

                  {/* Output / Console */}
                  <div className={`mt-3 rounded-md p-3 text-sm ${darkMode ? "bg-black bg-opacity-50 border border-gray-800 text-gray-100" : "bg-yellow-50 border border-yellow-200 text-gray-900"}`}>
                  ...
                  <div className={`mt-3 text-sm ${darkMode ? "text-gray-300" : "text-gray-700"}`}>
                    <strong className={darkMode ? "text-yellow-200" : "text-yellow-700"}>Explanation:</strong>
